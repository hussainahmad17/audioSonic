...
const addAudio = async (req, res) => {
    ...
    if (missingFields.length > 0 || !audioFile) {
        return res.status(400).json({ message: `Missing required fields: ${[...missingFields, ...(!audioFile ? ['audioFile'] : [])].join(', ')}` });
    }
    ...
    try {
        ...
        if (req.file && req.file.buffer) {
            try { const metadata = await mm.parseBuffer(req.file.buffer, { mimeType: req.file.mimetype }); duration = Math.round(metadata.format.duration || 0); } catch (err) { console.error("Duration extraction error:", err); duration = 0; }
            try { const filename = `${Date.now()}${path.extname(req.file.originalname)}`; const result = await uploadAudioBuffer(req.file.buffer, filename, req.file.mimetype); uploadedUrl = result.url; if (!duration && result.duration) { duration = Math.round(result.duration); } } catch (err) { console.error("Cloudinary upload error:", err); return res.status(500).json({ message: "Failed to upload audio" }); }
        }

        const newAudio = await FreeAudio.create({ title, description, rating: parseFloat(rating), categoryId, subCategoryId, audioFile: uploadedUrl || audioFile, language, voice, duration: duration });
        return res.status(201).json({ message: "Audio uploaded successfully", audio: newAudio });
    } catch (error) { console.error("Error handling free audio post:", error); return res.status(500).json({ message: "Internal server error", error: error.message }); }
}
...
const updateAudio = async (req, res) => {
    ...
    try {
        ...
        if (req.file && req.file.buffer) {
            try { const filename = `${Date.now()}${path.extname(req.file.originalname)}`; const result = await uploadAudioBuffer(req.file.buffer, filename, req.file.mimetype); updateData.audioFile = result.url; } catch (err) { console.error("Cloudinary upload error:", err); return res.status(500).json({ message: "Failed to upload audio" }); }
        }

        const updatedAudio = await FreeAudio.findByIdAndUpdate(id, updateData, { new: true, runValidators: true }).populate("categoryId", "categoryName").populate("subCategoryId", "Name");
        if (!updatedAudio) return res.status(404).json({ message: "Audio not found" });
        return res.status(200).json({ success: true, message: "Audio updated successfully", data: updatedAudio });
    } catch (error) { console.error("Error updating free audio:", error); return res.status(500).json({ message: "Internal server error" }); }
}

const deleteAudio = async (req, res) => {
    const { id } = req.params;
    try { const deletedAudio = await FreeAudio.findByIdAndDelete(id); if (!deletedAudio) return res.status(404).json({ message: "Audio not found" }); return res.status(200).json({ message: "Audio deleted successfully" }); } catch (error) { console.error("Error deleting free audio:", error); return res.status(500).json({ message: "Internal server error" }); }
}

const sendAudioEmail = async (req, res) => {
    const { email, audioTitle, audioDescription, audioUrl } = req.body;
    if (!email || !audioUrl || !audioTitle) return res.status(400).json({ success: false, message: "Missing required fields: email, audioUrl, or audioTitle." });
    try {
    ...
    } catch (error) { console.error('❌ Email send error:', error); return res.status(500).json({ success: false, message: "Failed to send email. Please try again later." }); }
};

const downloadAudios = async (req, res) => { return res.status(410).json({ message: "Download via direct URL is now supported. No local files." }); };

module.exports = { addAudio, getAllAudios, updateAudio, deleteAudio, sendAudioEmail, downloadAudios };
