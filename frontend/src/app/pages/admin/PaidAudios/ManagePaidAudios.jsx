...
import {
  ...
  MessageSquare,
  DollarSign,
} from "lucide-react"; // Added DollarSign icon
import { Link } from "react-router-dom";
import { deleteRequest, getRequest, putRequest } from "@app/backendServices/ApiCalls";
import { useApiSWR } from "@app/_hooks/useApiSWR";

const ManagePaidAudio = () => {
  // State management
  const { data: audiosRes, isLoading: loadingAudios, mutate: mutateAudios } = useApiSWR("/paid-audio");
  ...
  const [formData, setFormData] = useState({
    ...
    language: "",
    voice: "",
    priceAmount: 0, // Added priceAmount field
    audioFile: null,
  });
  ...
  const handleExportCSV = () => {
    const csvContent = [
      // Added "Price" column
      ["Title", "Description", "Rating", "Category", "Subcategory", "Price", "Language", "Voice", "Created At"].join(","),
      ...filteredAudios.map((audio) =>
        [
          ...
          audio.subCategoryId?.Name || subcategoryMap[audio.subCategoryId] || "N/A",
          audio.priceAmount || 0, // Added price amount
          audio.language || "N/A",
          audio.voice || "N/A",
    ...
    a.href = url;
    a.download = "paid_audio_list.csv";
    a.click();
  ...
  const filteredAudios = audios.filter(
    (audio) => audio && (
      ...
      (audio.subCategoryId?.Name || subcategoryMap[audio.subCategoryId] || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (audio.language || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (audio.voice || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (audio.priceAmount?.toString() || "").includes(searchQuery) // Search by price
    )
  );
  ...
  const handleOpenEditDialog = async (audio) => {
    ...
    // Set initial form data with priceAmount
    const initialFormData = {
      title: audio.title,
      description: audio.description,
      rating: audio.rating,
      categoryId: audio.categoryId?._id || audio.categoryId || "",
      subCategoryId: audio.subCategoryId?._id || audio.subCategoryId || "",
      language: audio.language || "",
      voice: audio.voice || "",
      priceAmount: audio.priceAmount || 0, // Added priceAmount
      audioFile: null,
    };
  ...
  const handleCloseEditDialog = () => {
    ...
    setFormData({
      ...
      language: "",
      voice: "",
      priceAmount: 0, // Reset priceAmount
      audioFile: null,
    });
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append("priceAmount", formData.priceAmount); // Added priceAmount

      if (formData.audioFile) {
        formDataToSend.append("audioFile", formData.audioFile);
      }

      const response = await putRequest(`/paid-audio/${selectedAudio._id}`, formDataToSend);
      if (response.success) {
  ...
  const handleDeleteConfirmed = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await deleteRequest(`/paid-audio/${audioToDelete._id}`);
  ...
  return (
    <Box>
      ...
      >
        ...
        >
          <TextField
            placeholder="Search audio, category, price..."
            value={searchQuery}
                ...
                paginatedAudios.map((audio, index) => (
                  audio && (
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip
                          label={`$${audio.priceAmount?.toFixed(2) || "0.00"}`}
                          size="small"
                          color="success"
                          variant="outlined"
                        />
      ...
      >
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 2 }}>
            ...
            <Box>
              <Typography variant="body2" color="text.secondary" gutterBottom>
                Audio File {"(Leave empty to keep current file)"}
              </Typography>
              <Button
                variant="outlined"
                component="label"
...
export default ManagePaidAudio;