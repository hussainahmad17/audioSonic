...
// cookie options (can adjust as needed)
const cookieOptions = {
  httpOnly: true,
...
const register = asyncHdlr(async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, cookieOptions); // ✅ set token in cookie
...
const verifyToken = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Error fetching user profile" });
  }
};
...
const GetAllUsers = async (req, res) => {
  try {
    ...
    referredUsers.forEach((refUser) => {
      referredMap[refUser._id.toString()] = refUser.username;
    });

    const result = users.map((user) => ({
      ...user._doc,
      referredBy: user.referredBy
        ? referredMap[user.referredBy.toString()] || null
        : null,
    }));
